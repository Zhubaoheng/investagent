...
Usage:
    echo '<header_json>\n<pdf_bytes>' | python -m investagent.datasources.pdf_extract_worker
"""
...
def _extract_markdown(raw_content: bytes) -> str:
    from investagent.datasources.pdf_extract import extract_pdf_markdown
    return extract_pdf_markdown(raw_content)


def _extract_sections(text: str, market: str) -> dict[str, str]:
    from investagent.datasources.pdf_extract import extract_sections
