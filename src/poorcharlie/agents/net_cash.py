...
from poorcharlie.schemas.net_cash import NetCashOutput
...
class NetCashAgent(BaseAgent):
    ...
    def _build_user_context(self, input_data: BaseModel, ctx: Any = None) -> dict[str, Any]:
        ...
        if ctx is not None:
            from poorcharlie.agents.context_helpers import data_for_net_cash, format_json
            data = data_for_net_cash(ctx)
