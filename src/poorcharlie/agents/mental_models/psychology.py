...
from poorcharlie.schemas.mental_models import PsychologyOutput
...
class PsychologyAgent(BaseAgent):
    ...
    def _build_user_context(self, input_data: BaseModel, ctx: Any = None) -> dict[str, Any]:
        ...
        if ctx is not None:
            from poorcharlie.agents.context_helpers import data_for_psychology, format_json
            data = data_for_psychology(ctx)
